...
```{r}
# Download the data for sweetgrass (Dicranum Scoparium).
Dicranum_scopariun <- BIEN_occurrence_species(species = "Dicranum scoparium")
```
...
```{r}
str(Dicranum_scopariun)
head(Dicranum_scopariun)
```

As you know from looking at the browser, you can look up a rangemap.

```{r}
moss_rangemap <- BIEN_ranges_load_species("Dicranum_scoparium")
```


```{r}
moss<-BIEN_occurrence_species(species = "Dicranum_scoparium")
```

```{r}
# Map the range of moss
map('world', fill=T, col="grey", bg="light blue", xlim=c(-180,20),ylim=c(-60,80))
plot(moss_rangemap, col="green", add=T)
```

```{r}
# Map the range of moss + occurence records
map('world', fill=T, col="grey", bg="light blue", xlim=c(-180,-20), ylim=c(-60, 80))
plot(moss_rangemap, col="green", add=T)
points(cbind(Dicranum_scopariun$longitude, Dicranum_scopariun$latitude), col="blue", pch=20, cex=1)
```
